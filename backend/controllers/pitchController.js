...
try {
  startupId = new mongoose.Types.ObjectId(req.user?.id || undefined);
} catch (err) {
  console.warn('Invalid or missing startupId. Using fallback.');
  startupId = new mongoose.Types.ObjectId(); // fallback for now
}
